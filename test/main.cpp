...
void check_region(const vmu::basic_region<Address> region, Address2 addr)
{
    ...
    CHECK(region.size() != 0);
    CHECK(vmu::detail::uintptr_cast(region.begin()) <=
          vmu::detail::uintptr_cast(addr));
    CHECK(vmu::detail::uintptr_cast(region.end()) >=
          vmu::detail::uintptr_cast(addr));
}
...
{
    auto random_data = std::make_unique<unsigned char[]>(4096 * 6);
    auto testing_arr = random_data.get();
    memset(testing_arr, 0xCC, 4096 * 6);
    auto result = vmu::query(testing_arr);
    REQUIRE(result);
    {
        ...
        vmu::protection_guard pg(
            testing_arr + 4097, testing_arr + 4096 * 3, new_prot);

        auto new_flags = vmu::query(testing_arr + 4097);
        CHECK(new_flags);
        CHECK(new_flags.protection().to_flags() == new_prot);
    }
